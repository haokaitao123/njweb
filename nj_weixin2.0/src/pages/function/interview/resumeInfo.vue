<template>
	<div class="resumeInfo">
		<div class="resumeInfoWrap" ref="abc">
			<div class="title title_f">
				<div class="title_left">
					<img src="../../../../static/function/resumeTitle.png" alt="">
					<h3>基本信息</h3>
				</div>
			</div>
			...
			<div class="title">
				<div class="title_left">
					<img src="../../../../static/function/educatInfo.png" alt="">
					<h3>教育信息<span>*</span></h3>
					<icon type="warn" class="error" v-show="educationState"></icon>
				</div>
				<span @click="goTo('educationShow')" v-if="!state">添加</span>
			</div>
			<group label-align="left" gutter="0" class="form childForm">
				<div class="item_box" v-for="item in educationList" v-if="educationList.length!==0">
					<cell :value=item.reeduSchool is-link @click.native="goTo('educationShow',item.id)"></cell>
				</div>
				<div class="item_box" v-if="educationList.length<1&&!state">
					<cell value='学历信息1' is-link @click.native="goTo('educationShow')"></cell>
				</div>
				<div class="item_box" v-if="educationList.length<2&&!state">
					<cell value='学历信息2' is-link @click.native="goTo('educationShow')"></cell>
				</div>
			</group>
			<div class="title">
				<div class="title_left">
				...
				<span @click="goTo('workExpShow')" v-if="!state">添加</span>
			</div>
			<group label-align="left" gutter="0" class="form childForm">
				<div class="item_box" v-for="item in workExpList" v-if="workExpList.length!==0">
					<cell :value=item.reweCompnm @click.native="goTo('workExpShow',item.id)" is-link></cell>
				</div>
				<div class="item_box" v-if="workExpList.length<1&&!state">
					<cell value='工作经历1' @click.native="goTo('workExpShow')" is-link></cell>
				</div>
				<div class="item_box" v-if="workExpList.length<2&&!state">
					<cell value='工作经历2' @click.native="goTo('workExpShow')" is-link></cell>
				</div>
				<div class="item_box" v-if="workExpList.length<3&&!state">
					<cell value='工作经历3' @click.native="goTo('workExpShow')" is-link></cell>
				</div>
			</group>
			<div class="title">
				<div class="title_left">
			...
			<group label-align="left" gutter="0" class="form childForm">
				<div class="item_box" v-for="item in familyList">
					<cell :value=item.refaMembersDis @click.native="goTo('familyShow',item.id)" is-link></cell>
				</div>
			</group>
			<div class="save_button">
				<x-button type="primary" class="x_button" :disabled="state" @click.native="save" action-type="button">保存</x-button>
			</div>
		</div>
		<!-- 身份 -->
		<van-popup v-model="relibIdentityShow" position="bottom">
			<van-picker ref="vanPicker" :defaultIndex="relibIdentityIndex" :columns=selectRelibIdentity show-toolbar @confirm="confirm" @cancel="cancel" />
		</van-popup>
		<!-- 性别 -->
		...
		<van-popup v-model="relibBirtdaydateShow" position="bottom">
			<van-datetime-picker v-model="relibBirtdayDate" type="date" :min-date="minRelibBirtday" :max-date="maxRelibBirtday" @confirm="confirm" @cancel="cancel" />
		</van-popup>
		<!-- 可到岗时间 -->
		<van-popup v-model="relibAvaitimedateShow" position="bottom">
			<van-datetime-picker v-model="relibAvaitimeDate" type="date" :min-date="minRelibAvaitime" :max-date="maxRelibAvaitime" @confirm="confirm" @cancel="cancel" />
		</van-popup>
		<!-- 邀约时间 -->
		...
		<van-popup v-model="relibApplypostShow" position="right" class="popup_width">
			<searchPost @inputPost="inputPost" :currentId="currentPostId" v-if="relibApplypostShow" ref="searchPost"></searchPost>
		</van-popup>
	</div>
</template>
<script>
