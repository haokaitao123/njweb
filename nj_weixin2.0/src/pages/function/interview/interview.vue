<template>
	<div class="interview">
		<van-pull-refresh v-model="isLoading" @refresh="onRefresh" class="interviewWrap" v-show="!addNewInterview">
			<van-list v-model="loading" :finished="finished" :finished-text="finishedText" @load="onLoad" :offset="10">
				<div class="interviewItem" v-for="(item,index) in list" @click="goTo(item.id)">
					...
					<div class="interview_item item_second">
						<div class="item_left">
							<img src="../../../../static/function/telephone.png" alt="">
					...
					<div class="button_box">
						<button type="button" @click="getQrCode($event,item.id)">预约二维码</button>
						<button type="button" v-if="item.curStepstate!=='p_flowst_3'&&item.curStepDis==='初试'" @click="submit($event,item.id)">提交</button>
						<button type="button" v-else-if="item.curStepDis==='复试'&&item.curStepstate==='p_flowst_3'" @click="interviewMes($event,item.id)">预约信息</button>
					</div>
				</div>
			</van-list>
		</van-pull-refresh>
		<div class="qrCodeContent" v-show="show">
			<canvas id="canvas" style="height:600px;width:300px"></canvas>
		</div>
		<div id="mask" @click="closeQrCode" v-show="maskShow"> </div>
		<div class="addNew" v-show="!addNewInterview">
			<span class="add" @click="goTo()">
				+
			</span>
		</div>
		<div class="interviewMes" v-show="interviewMesShow">
		...
		<addNewInterview v-show="addNewInterview" :btnName='btnName'
	...
	export default {
		...
		methods: {
			...
			getQrCode(e, id) {
				...
				const url = 'http://192.168.151.26:8080/#/resumeInfo?id=' + id
