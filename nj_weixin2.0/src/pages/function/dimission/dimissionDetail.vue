	...
	<div class="dimissionDetail">
		<div class="dimissionDetailWrap">
			<group label-align="left" gutter="0" class="form">
				<!-- 申请人-->
				<div class="item_box">
					<x-input title="申请人" v-model="dimApplicantDis" :disabled="true" :show-clear="false" placeholder="">
					</x-input>
				</div>
				<!-- 申请时间 -->
				<div class="item_box">
					<x-input title="申请时间" v-model="form.dimApplydate" :show-clear="false" :disabled="true" placeholder="">
					</x-input>
				</div>
				<!-- 员工姓名 -->
				<div class="item_box">
					<cell title="" is-link value-align="left" v-model="empIdName" v-verify="form.empId" @click.native="popupClick('empShow','empId')">
						<div slot="title">员工姓名<span>*</span></div>
					</cell>
					<icon type="warn" class="error" v-show="empIdName=='请选择'?true:false" v-remind="form.empId"></icon>
				</div>
				<!-- 证件号码 -->
				<div class="item_box">
					<x-input title="证件号码" v-model="form.empIdno" :disabled="true" :show-clear="false" placeholder="未填写">
					</x-input>
				</div>
				<!-- 部门名称 -->
				<div class="item_box">
					<x-input title="部门名称" v-model="deptIdDis" :disabled="true" :show-clear="false" placeholder="未填写">
					</x-input>
				</div>
				<!-- 岗位名称 -->
				<div class="item_box">
					<x-input title="岗位名称" v-model="postIdDis" :disabled="true" :show-clear="false" placeholder="未填写">
					</x-input>
				</div>
				<!-- 上级经理 -->
				<div class="item_box">
					<x-input title="上级经理" v-model="dimBuspmpDis" :disabled="true" :show-clear="false" placeholder="未填写">
					</x-input>
				</div>
				<!-- 手机号码 -->
				<div class="item_box">
					<x-input title="手机号码" v-model="form.empnhMobile" :disabled="true" :show-clear="false" placeholder="未填写">
					</x-input>
				</div>
				<!-- 银行账号-->
				<div class="item_box">
					<x-input title="银行账号<span>*</span>" v-model="form.empnhSalaccount" v-verify="form.empnhSalaccount" :show-clear="false"
					 placeholder="请填写">
					</x-input>
					<icon type="warn" class="error" v-show="form.empnhSalaccount==''?true:false" v-remind="form.empnhSalaccount"></icon>
				</div>
				<!-- 是否需要离职证明 -->
				<div class="item_box">
					<cell title="" is-link value-align="left" v-model="dimCertifiDis" @click.native="popupClick('dimCertifiShow','dimCertifi')">
						<div slot="title">是否需要离职证明</div>
					</cell>
				</div>
				<!-- 个人邮箱 -->
				<div class="item_box">
					<x-input title="个人邮箱<span>*</span>" v-model="form.dimPersmail" v-verify="form.dimPersmail" :show-clear="false"
					 placeholder="请填写">
					</x-input>
					<icon type="warn" class="error" v-show="form.dimPersmail==''?true:false" v-remind="form.dimPersmail"></icon>
				</div>
				<!-- 离职类型 -->
				<div class="item_box">
					<cell title="" is-link value-align="left" v-model="dimTypeDis" v-verify="form.dimType" @click.native="popupClick('dimTypeShow','dimType')">
						<div slot="title">离职类型<span>*</span></div>
					</cell>
					<icon type="warn" class="error" v-show="dimTypeDis=='请选择'?true:false" v-remind="form.dimType"></icon>
				</div>
				<!-- 离职原因 -->
				<div class="item_box">
					<cell title="" is-link value-align="left" v-model="dimReasonDis" v-verify="form.dimReason" @click.native="popupClick('dimReasonShow','dimReason')">
						<div slot="title">离职原因<span>*</span></div>
					</cell>
					<icon type="warn" class="error" v-show="dimReasonDis=='请选择'?true:false" v-remind="form.dimReason"></icon>
				</div>
				<!-- 申请离职日期 -->
				<div class="item_box">
					<cell title="" is-link value-align="left" v-model="form.dimLevsqday" v-verify="form.dimLevsqday" @click.native="popupClick('dimLevsqdayShow','dimLevsqday')">
						<div slot="title">申请离职日期<span>*</span></div>
					</cell>
					<icon type="warn" class="error" v-show="form.dimLevsqday=='请选择'?true:false" v-remind="form.dimLevsqday"></icon>
				</div>
				<!-- 约定离职日期 -->
				<div class="item_box">
					<cell title="" is-link value-align="left" v-model="form.dimLevday" v-verify="form.dimLevday" @click.native="popupClick('dimLevdayShow','dimLevday')">
						<div slot="title">约定离职日期<span>*</span></div>
					</cell>
					<icon type="warn" class="error" v-show="form.dimLevday=='请选择'?true:false" v-remind="form.dimLevday"></icon>
				</div>
				<!-- 最后工作日期 -->
				<div class="item_box">
					<cell title="" is-link value-align="left" v-model="form.dimLaswkday" v-verify="form.dimLaswkday" @click.native="popupClick('dimLaswkdayShow','dimLaswkday')">
						<div slot="title">最后工作日期<span>*</span></div>
					</cell>
					<icon type="warn" class="error" v-show="form.dimLaswkday=='请选择'?true:false" v-remind="form.dimLaswkday"></icon>
				</div>
				<!-- 是否工作交接 -->
				<div class="item_box">
					<cell title="" is-link value-align="left" v-model="dimIsreceiveDis" @click.native="popupClick('dimIsreceiveShow','dimIsreceive')">
		...
		<!-- 员工 -->
		<van-popup v-model="empShow" position="right" class="right_popup">
			<searchEmp @inputEmp="inputEmp" :currentId="currentId"></searchEmp>
		</van-popup>
		<!-- 是否需要离职证明 -->
		<van-popup v-model="dimCertifiShow" position="bottom">
			<van-picker ref="dimCertifiPicker" :defaultIndex="dimCertifiIndex" :columns=selectDimCertifi show-toolbar @confirm="confirm"
			 @cancel="cancel" />
		</van-popup>
		<!-- 离职类型 -->
		<van-popup v-model="dimTypeShow" position="bottom">
			<van-picker ref="dimTypePicker" :defaultIndex="dimTypeIndex" :columns=selectDimType show-toolbar @confirm="confirm"
			 @cancel="cancel" />
		</van-popup>
		<!-- 离职原因 -->
		<van-popup v-model="dimReasonShow" position="bottom">
			<van-picker ref="dimReasonPicker" :defaultIndex="dimReasonIndex" :columns=selectDimReason show-toolbar @confirm="confirm"
			 @cancel="cancel" />
		</van-popup>
		<!-- 是否工作交接 -->
		<van-popup v-model="dimIsreceiveShow" position="bottom">
			<van-picker ref="dimIsreceivePicker" :defaultIndex="dimIsreceiveIndex" :columns=selectDimIsreceive show-toolbar
			 @confirm="confirm" @cancel="cancel" />
		</van-popup>
		<!-- 申请离职日期 -->
		<van-popup v-model="dimLevsqdayShow" position="bottom">
			<van-datetime-picker v-model="dimLevsqdayDate" type="date" :min-date="minDimLevsqday" :max-date="maxDimLevsqday"
			 @confirm="confirm" @cancel="cancel" />
		</van-popup>
		<!-- 约定离职日期 -->
		<van-popup v-model="dimLevdayShow" position="bottom">
			<van-datetime-picker v-model="dimLevdayDate" type="date" :min-date="minDimLevday" :max-date="maxDimLevday" @confirm="confirm"
			 @cancel="cancel" />
		</van-popup>
		<!-- 最后工作日期 -->
		<van-popup v-model="dimLaswkdayShow" position="bottom">
			<van-datetime-picker v-model="dimLaswkdayDate" type="date" :min-date="minDimLaswkday" :max-date="maxDimLaswkday"
			 @confirm="confirm" @cancel="cancel" />
		</van-popup>
	</div>
</template>
