<template>
    <div class="attenGooutproc">
        <van-pull-refresh v-model="isLoading"
                          @refresh="onRefresh"
                          v-show="!noAttenGooutproc"
                          class="attenGooutprocWrap">
            <van-list v-model="loading"
                      :finished="finished"
                      :finished-text="finishedText"
                      @load="onLoad"
                      :offset="10">
                <div class="attenGooutprocItem"
                     @click="goTo(item.id)"
                     v-for="(item,index) in list">
                    <div class="item_left">
                        <span>外出员工：{{item.empnhName}}</span>
                        <span>
                            公<b>公司</b>司：{{companyName}}
                        </span>
                        <span>部<b>公司</b>门：{{item.unitFname}}</span>
                        <span>岗<b>公司</b>位：{{item.postFname}}</span>
                        <span>外出日期：{{item.goutBgdate}}</span>
        ...
        <noAttenGooutproc v-show="noAttenGooutproc"
                          :btnName='btnName'
                          @addNew="applyAttenGooutproc"></noAttenGooutproc>
    </div>
</template>
<script>
import noAttenGooutproc from '@/components/public/addNew'
...
export default {
    ...
    methods: {
        goTo (id) {
            this.$router.push({ name: 'attenGooutprocDetail', query: { id: id } })
        },
        //上拉加载
        onLoad () {
