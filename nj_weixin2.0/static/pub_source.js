const pubsource = {
  // 以下为开发环境变量配置
  // pub_getlink: "http://najie.hrocloud.com:8099/api/v1",
  // pub_getCaptcha: "http://najie.hrocloud.com:8099/api/v1",
  // pub_pubf_uploadlink: "http://najie.hrocloud.com:9709/upload?groupId=0",
  // pub_prvf_uploadlink: "http://najie.hrocloud.com:9709/upload?groupId=1",
  // pub_pubf_downlink: "http://najie.hrocloud.com:8088/",
  // pub_prvf_downlink: "http://najie.hrocloud.com:9709/file?",
	// pub_url:"http://192.168.151.26:8080/",
  clientId: '1000',
  clientPass: '6cc1c430aed64d3bb8516011195a5b56',
  webSocketURL: 'ws://192.168.11.66:3048/ws',
  // 以下为测试环境变量配置，新变量请同开发增加
    pub_getlink: "http://hr.333job.com:9102/api/v1",
    pub_getCaptcha: "http://hr.333job.com:9102/api/v1",
    pub_pubf_uploadlink: "http://hr.333job.com:9101/upload?groupId=0",
    pub_prvf_uploadlink: "http://hr.333job.com:9101/upload?groupId=1",
    pub_pubf_downlink: "http://hr.333job.com:9100/",
    pub_prvf_downlink: "http://hr.333job.com:9101/file?",
	  pub_url:"http://hr.333job.com:9001/",

  // 以下为生产环境变量配置，新变量请同开发增加

  // 以下为该变量收尾，新增的请放到该变量前面
  webSocketDevice: 'weixin',
  applicationId: '70',
  companyId: '1000',
  companyName: '纳杰集团总部',
  corpId: 'wwce608d73697f42d2',
  appSecret: '2qKxvpzR7codeE1sj7lbRh-RtuWpr3E5GW2StdbIewY',
  pub_pubsources_end: '该变量收尾，新增的请放到该变量前面',
};
// export default pubsource
