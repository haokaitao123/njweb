<template>
  <div class="main">
      ...
      <div class="header-avator-con">
        <div class="user-dropdown-menu-con">
          <Row type="flex" justify="end" align="middle">
            <Poptip  class="mainPop" placement="bottom" width="300" style="display: none">
            ...
            <!--<Poptip trigger="hover" class="mainPop" placement="bottom" width="300" >-->
              ...
              <!--</message>-->
            <!--</Poptip>-->
            <div class="button-item" v-show="roleTypes.length>1">
              <Dropdown transfer trigger="hover" @on-click="changeRole">
                <a href="javascript:void(0)" style="display: block;width: 100%;height: 47px;line-height: 47px;">
                  <Icon type="android-desktop" style="color: white;"></Icon>
                  <span class="main-user-name">{{ cur_roleTypeName }}</span>
                </a>
                <DropdownMenu slot="list">
                  <DropdownItem :name="item.roleTypeCode + ',' + item.roleTypeName" v-for="(item,index) in roleTypes" :key="index">{{item.roleTypeName}}</DropdownItem>
                </DropdownMenu>
              </Dropdown>
            </div>
            <div class="button-item" v-show="companyies.length>1">
              <Dropdown transfer trigger="hover" @on-click="changeCompany">
                <a href="javascript:void(0)" style="display: block;width: 100%;height: 47px;line-height: 47px;">
                  <span class="main-user-name">{{ cur_companyName }}</span>
                  <Icon type="arrow-down-b"></Icon>
                </a>
                <DropdownMenu slot="list">
                  <DropdownItem :name="item.companyId+','+item.companyName" v-for="(item,index) in companyies" :key="index">{{item.companyName}}</DropdownItem>
                </DropdownMenu>
              </Dropdown>
            </div>
            <div class="button-item">
              <Dropdown transfer trigger="hover" @on-click="changeLanguage">
                <a href="javascript:void(0)" style="display: block;width: 100%;height: 47px;line-height: 47px;">
                  <span class="main-user-name">{{ cur_languageName }}</span>
                  <Icon type="arrow-down-b"></Icon>
                </a>
                <DropdownMenu slot="list">
                  <DropdownItem :name="item.lanCode+','+item.lanName" v-for="(item,index) in languages" style="padding-left: 35px;position: relative;" :key="index">
                    <img :src="'../../static/homeImg/' + item.lanCode + '.png'" style="position: absolute;left: 8px;top: 8px;" />
                    {{item.lanName}}
                  </DropdownItem>
                </DropdownMenu>
              </Dropdown>
            ...
            <Avatar icon="person" :src="userImg" style="background: #619fe7;margin-left:10px;margin-right: 10px;"></Avatar>
          </Row>
        </div>
      </div>
    </div>
    <div class="section" v-show="!showUserInfo"

    >
      <hrmenu class="left" @toggleClick="toggleClick" ref="menu"></hrmenu>
      <div class="right" :style="{left: shrink?'64px':'200px'}">
        <pagetab class="pagetab" :pageTagsList="pageTagsList"></pagetab>
    ...
    <userinfo v-show="showUserInfo" :isEmpty="isEmpty" ref="userinfo" @close="close"></userinfo>
    <transition name="fade">
    	 <loginim v-show="showIM" ref="loginim" @closeIM="closeIM"></loginim>
    </transition>
    <el-dialog :title="$t('lang_homePage.platSetting.title')" style="margin: 0 auto !important;"  width='1000px' :visible.sync="openUpdate" @close="closeAppro">
      <Form ref="form" :model="form" :rules="ruleValidate" :label-width="135" style="border-radius: 10px !important;" >
      <Row style="display: flex;align-items: flex-start">
        <i-col span="11">
          <FormItem :label="$t('lang_homePage.platSetting.platApvset')" prop="platApvset">
            <Select v-model="form.platApvset" style="width:300px;">
              <Option v-for="(item,index) in Repeattype" :key="index" :value="item.paramCode">{{item.paramInfoCn}}</Option>
            </Select>
          </FormItem>
        </i-col>
        <i-col span="10" offset="1" style="padding-top: 5px">
          <span style="font-size: 12px" >{{$t('lang_homePage.platSetting.apMethod')}}</span>
        </i-col>
      </Row>
      <Row style="text-align: right">
        <Button type="ghost" @click="closeAppro">{{$t('button.cal')}}</Button>
        <Button type="primary" @click="savePlatApvset">{{$t('button.sav')}}</Button>
      </Row>
      </Form>
    </el-dialog>
  </div>
