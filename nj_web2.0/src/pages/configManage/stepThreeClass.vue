<template>
  ...
  <div>
    ...
    <transition name="fade">
      <addClass ref="addClass" v-if="showAdd" @cancle="cancle" @getData="addNewArray" @update="updateArray"
                :id="updateId" :index="index" :logType="logType" :suvid="suvid" :platQusCatid="platQusCatid"
                :styleCode="styleCode" @getClassid="getClassid($event)"></addClass>
    </transition>
  </div>
</template>
  ...
  export default {
    data() {
      return {
        ...
        searchColumn: [
          ...
          {
            title: this.$t('lang_role.suvadmin.platCatDesccn'),
            key: 'platCatDesccn',
            sortable: 'custom',
          },
          {
            title: this.$t('lang_role.suvadmin.platCatDescen'),
            key: 'platCatDescen',
            sortable: 'custom',
          },
          {
    ...
    methods: {
      ...
      getData() {
        const t = this
        const data = {
          _mt: 'platSurveycategory.getPage',
          sort: t.sort,
          order: t.order,
          rows: t.rows,
          page: t.page,
          logType: this.$t('button.ser'),
          platCatSuvid: t.suvid,
        }
      ...
      delData() {
        ...
        if (t.updateArr.length === 0) {
          ...
          t.$Modal.confirm({
            ...
            onOk: () => {
              const data = {
                _mt: 'platSurveycategory.delByIds',
      ...
      addNew(id, logType, index) {
        ...
        t.logType = logType
        t.index = index
        // t.$refs.addClass.form.platCatSuvid = parseInt(t.suvid, 10)
        // if (logType === this.$t('button.upd')) {
        //     t.$refs.addClass.upData(id)
        // }
      },
      // 关闭新增编辑信息页面
      cancle() {
      ...
      getClassid(id) {
        ...
        this.$emit('getClassid', this.platQusCatid)
      },
    },
  }
</script>
