...
<template>
  <div style="width: 100%;">
    <div id="EchatsFour" ref="EchatsFour" style="width: 100%;height: 350px"></div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      getDataPieNum() {
        const t = this
        const data = {
          _mt: 'ansrptFlowStatistic.getFlowStatisticListByHircomp',
          logType: '按照公司统计业务流程总数量、已完成数量',
//          userId: t.$store.state.user.userId,
        ...
        getDataLevelUserLoginNew(data).then((res) => {
          if (isSuccess(res, t)) {
            ...
            Value.forEach((item) => {
              console.log(item.hirecompanyName)
              this.companyName.push(item.hirecompanyName)
      ...
      getEchatsColumnNum(data) {
        ...
        let echats3 = this.$echarts.init(document.getElementById('EchatsFour'))
        echats3.setOption({
          title: {
//            text: "知识分类统计",
            textStyle: {
