      ...
      <row>
        <i-col class="meau-left" span="5">
          <Menu :active-name="active" width="auto" @on-select="pageTo">
            <!-- <MenuItem name="updPlatDocTemp">{{$t('lang_platdoc.platDoc.title1')}}</MenuItem>
            <MenuItem
              name="docTempVar"
              v-show="$store.state.docTemp.mainId"
            >{{$t('lang_platdoc.platDoc.title2')}}</MenuItem>
            </Menu>-->
            <MenuItem name="updPlatDocTemp">员工基础信息</MenuItem>
            <MenuItem name="docTempVar">学历信息管理</MenuItem>
          </Menu>
        </i-col>
