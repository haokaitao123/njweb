    ...
    <Row>
      ...
      <card>
        <p slot="title">
          ...
          &nbsp;{{$t('lang_sihfCalc.sihfHfcalc.title')}}
        </p>
        <Row>
          <span @dblclick="clearPeriod">
            <Input @on-click="pickDataPeriod" icon="search" :readonly="true"
                   :placeholder="$t('lang_sihfCalc.sihfHfcalc.sihfcalcPeriod')"
                   style="width: 200px" v-model="sihfcalcPeriodDis"/>
          </span>
          <span @dblclick="clearBfarea">
            <Input @on-click="pickDataBfarea" icon="search" :readonly="true"
                   :placeholder="$t('lang_sihfCalc.sihfHfcalc.sihfcalcBfarea')"
                   style="width: 200px" v-model="sihfcalcBfareaDis"/>
          </span>
          <Dropdown>
            <Button type="primary">
              {{select}}
            ...
            <DropdownMenu slot="list">
              ...
              <span v-for="(item,index) in calcround" :key="index" @click="selected(item.paramCode,item.paramInfoCn)">
          ...
          <Button v-show="this.sihfcalcStatus === '04confirmed' && this.sihfcalcRound === '90final' ? true:false"
                  type="primary"
                  @click="submit('05alreadySealed')">{{$t('button.sealAccount')}}
          </Button>
          <Button v-show="this.sihfcalcStatus === '02accounting'? true:false" type="error" @click="deletemsg">
            {{$t('button.del')}}
          </Button>
        </Row>
        <Row>
          <RadioGroup v-model="sihfcalcStatus" v-for="(item,index) in calculation" :key="index" style="margin-top: 5px"
  ...
  export default {
    data() {
      return {
        ...
        columns: [
          ...
          {
            title: this.$t('lang_sihfCalc.sihfHfcalc.sihfcalcPeriod'),
            key: 'sihfcalcPeriodDis',
            width: 150,
            sortable: 'custom',
          },
          {
            title: this.$t('lang_sihfCalc.sihfHfcalc.sihfcalcBfarea'),
            key: 'sihfcalcBfareaDis',
            width: 150,
            sortable: 'custom',
          },
          {
            title: this.$t('lang_sihfCalc.sihfHfcalc.sihfcalcRound'),
            key: 'sihfcalcRoundDis',
            width: 150,
            sortable: 'custom',
          },
          {
            title: this.$t('lang_sihfCalc.sihfHfcalc.sihfcalcPersttamt'),
            key: 'sihfcalcPersttamt',
            width: 150,
            sortable: 'custom',
          },
          {
            title: this.$t('lang_sihfCalc.sihfHfcalc.sihfcalcCompttamt'),
            key: 'sihfcalcCompttamt',
            width: 150,
            sortable: 'custom',
          },
          {
            title: this.$t('lang_sihfCalc.sihfHfcalc.sihfcalcTtamt'),
            key: 'sihfcalcTtamt',
            width: 150,
            sortable: 'custom',
          },
          {
            title: this.$t('lang_sihfCalc.sihfHfcalc.sihfcalcStatus'),
            key: 'sihfcalcStatusDis',
            width: 150,
            fixed: 'right',
            sortable: 'custom',
          },
            ...
            render: (h, params) => {
              return h('div', [
                ...
                h('Button', {
                  ...
                  style: {
                    marginLeft: '5px',
                    display: this.sihfcalcStatus === '02accounting' ? 'inline' : 'none',
                  },
                  on: {
                    click: () => {
                ...
                h('Button', {
                  ...
                  style: {
                    marginLeft: '5px',
                    // display: params.row.sihfcalcRound == '90final' && this.sihfcalcStatus == '04confirmed' ?'inline' : 'none',
                  },
                  on: {
                    click: () => {
    ...
    methods: {
      getData(page) {
        ...
        const data = {
          _mt: 'sihfHfcalc.getPage',
          sort: t.sort,
          order: t.order,
          rows: t.rows,
          page: t.page,
          logType: '查询List',
          sihfcalcPeriod: t.sihfcalcPeriod,
          sihfcalcBfarea: t.sihfcalcBfarea,
          sihfcalcRound: t.sihfcalcRound,
          sihfcalcStatus: t.sihfcalcStatus,
        }
      ...
      getPageByState(index, message) {
        const t = this
        this.$store.commit('hfcalcJsBase/setStatus', t.sihfcalcStatus)
        if (t.sihfcalcStatus === '02accounting') {
          t.openStateAdd = true
          t.openStateSub = true
          t.openStateCon = false
        } else if (t.sihfcalcStatus === '03ToBeConfirmed') {
          t.openStateSub = false
          t.openStateAdd = false
          t.openStateCon = true
        } else if (t.sihfcalcStatus === '04confirmed') {
          t.openStateCon = false
          t.openStateAdd = false
          t.openStateSub = false
        } else if (t.sihfcalcStatus === '01all') {
          t.openStateAdd = true
          t.openStateCon = false
          t.openStateSub = false
        }
      ...
      initSetState() {
        this.$store.commit('hfcalcJsBase/setStatus', '01all')
      },
      sizeChange(size) {
        const t = this
        t.rows = size
      ...
      openUp(row, logType, index) {
        ...
        t.openUpdate = true
        if (row === '') {
          this.$store.commit('hfcalcJsBase/setDataType', 'add')
        } else {
          this.$store.commit('hfcalcJsBase/setDataType', 'sel')
        }
        this.$store.commit('hfcalcJsBase/setIndex', index)
        if (row.id) {
          this.$store.commit('hfcalcJsBase/setMainId', row.id)
...
//      将本条数据的id传入mainid（如果有的话）
      ...
      closeUp() {
        ...
        this.$store.commit('hfcalcJsBase/setIndex', 0)
        this.$store.commit('hfcalcJsBase/setGrandId1', '')
        this.$store.commit('hfcalcJsBase/setChildId1', '')
      },
      addNewArray(res) {
        const t = this
        t.openUpdate = false
      ...
      updateArray(res) {
        const t = this
        t.index = this.$store.state.hfcalcJsBase.index
      ...
      countSihfCalc(row, logType) {
        const t = this
        const data = {
          _mt: 'sihfHfcalc.addComputeCalc',
          logType: '查询List',
          sihfcalcStatus: t.sihfcalcStatus,
          sihfcalcBfarea: row.sihfcalcBfarea,
          id: row.id,
          sihfcalcPeriod: row.sihfcalcPeriod,
        }
        for (const dat in data) {
          if (data[dat] === '') {
            delete data[dat]
        ...
        getDataLevelUserLoginNew(data).then((res) => {
          if (isSuccess(res, t)) {
            ...
            t.$Modal.success({
              ...
              content: this.$t('lang_repayApply.base.psirpSq'),
              onOk: () => {
                t.isSend = true
...
//                t.imp_status = res.data.content[0].imp_status
      ...
      changeinputPeriod(row) {
        const t = this
        t.sihfcalcPeriodDis = row.fypdPeriod
        t.sihfcalcPeriod = row.id
      },
      // 期间弹出框end
      // 区域弹出框start
      clearBfarea() {
      ...
      changeinputBfarea(row) {
        const t = this
        t.sihfcalcBfareaDis = row.bfareaCname
        t.sihfcalcBfarea = row.id
      },
      // 区域弹出框end
      submit(status) {
        const t = this
      ...
      getSelect() {
        ...
        getDataLevelUserLogin({
          ...
          if (isSuccess(res, t)) {
            this.$store.commit('hfcalcJsBase/setCalcround', res.data.content[0].value[0].paramList)
