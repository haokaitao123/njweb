        ...
        <i-col class="meau-left" span="5">
          <Menu :active-name="active" width="auto" @on-select="pageTo">
            <MenuItem name="option">
              {{$t('lang_sihfPolicy.sihtPolicy.title')}}
            </MenuItem>
            <MenuItem name="content" v-show="$store.state.sihfPo.childId1">
              {{$t('lang_sihfPolicy.sihtPlcyinsurance.title')}}
            </MenuItem>
          </Menu>
        </i-col>
...
<script>
  import mOption from './sihfPolicyView'
  import mContent from './sihfPlcyinsuranceManage'
  ...
  export default {
    ...
    methods: {
      ...
      getOption(logType) {
        this.data.id = this.$store.state.sihfPo.mainId
