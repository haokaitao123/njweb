  ...
  <div class="option-main">
    <Row style="max-height: 420px;overflow-y: auto;">
      <Form :model="form" label-position="right"  ref="form" :label-width="120">
        ...
        <i-col span="11">
          <FormItem :label="$t('lang_ansrptEmpSihf.ansrptEmpSihf.empbntBenefitlocation')" >
        ...
        <i-col span="11" offset="1" >
          <FormItem :label="$t('lang_ansrptEmpSihf.ansrptEmpSihf.empbntBenefitpolicy')" >
            <Input v-model="form.empbntBenefitpolicy"disabled
                   :placeholder="$t('lang_ansrptEmpSihf.ansrptEmpSihf.empbntBenefitpolicy')"></Input>
          </FormItem>
        </i-col>
        <i-col span="11" >
          <FormItem :label="$t('lang_ansrptEmpSihf.ansrptEmpSihf.empbntBenefittype')"  >
  ...
  export default {
    ...
    methods: {
      getdata() {
        const t = this
        const params = {
          _mt: 'ansrptEmpSihfBenefit.getById',
          id: t.$store.state.ansrptEmpSihf.mainId,
          logType: '根据id查询',
        }
        getDataLevelUserLogin(params).then((res) => {
          if (isSuccess(res, t)) {
            this.form.empbntEmpid = res.data.content[0].empCname
            this.form.empbntHirecompany = res.data.content[0].empbntHirecompanyDis
            this.form.empbntBenefitlocation = res.data.content[0].empbntBenefitlocationDis
            this.form.empbntBenefitpolicy = res.data.content[0].empbntBenefitpolicyDis
            this.form.empbntBenefittype = res.data.content[0].empbntBenefittypeDis
            this.form.empbntSdate = res.data.content[0].empbntSdate
            this.form.empbntEdate = res.data.content[0].empbntEdate
            this.form.empbntComment = res.data.content[0].empbntComment
          }
        }).catch(() => {
          t.$Modal.error({
