      ...
      <row>
        <i-col class="meau-left" span="5">
          <Menu :active-name="active" width="auto" @on-select="pageTo">
            <MenuItem name="option">
              {{$t('lang_sihfPolicy.sihfBenefitadd.baseTitle')}}
            </MenuItem>
            <MenuItem name="content" v-show="id">
              {{$t('lang_sihfPolicy.sihfBenefitadd.subTitle')}}
            </MenuItem>
          </Menu>
        </i-col>
        <i-col class="meau-right" span="19">
          ...
          <div style="margin-top: 40px;padding: 10px;">
            <mOption v-show="option" :logType="logType" ref="option" @getEntryDate="getEntryDate" @getStatus="getStatus" @handleReset="handleReset" :id="id" @update="update" @newdata="newdata"></mOption>
...
<script>
  import mOption from './editBenefitadd'
  ...
  export default {
    ...
    methods: {
      ...
      getEntryDate(date) {
        this.sibaEntrydate = date
      },
      getStatus(status) {
        this.sibaStatus = status
      },
      pageTo(name) {
        this.option = false
        this.content = false
        ...
        this[name] = true
        if (name === 'content') {
          this.$refs.content.get(this.id, this.sibaEntrydate, this.sibaStatus)
        } else {
