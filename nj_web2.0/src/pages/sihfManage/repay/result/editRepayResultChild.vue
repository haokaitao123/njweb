        ...
        <Form :model="form" ref="form" label-position="right"  :label-width="140">
          ...
          <i-col span="23">
            <FormItem :label="$t('lang_repayApply.sihfRepayInsurance.sirpinsComment')" prop="comment">
              <Input v-model="form.sirpinsComment"disabled type="textarea" :autosize="{minRows: 2,maxRows: 5}" :placeholder="$t('lang_authorization.platRole.commentDis')"></Input>
            </FormItem>
          </i-col>
        </Form>
      </Row>
...
//        if (t.$store.state.repayResult.mainId) {
//          data.sirpinsRpid = t.$store.state.repayResult.mainId
//          data.id = t.rowId
//        }
//        data.logType = '修改'
...
//          if (isSuccess(res, t)) {
//            t.$store.commit('repayResult/setChildId1', res.data.content[0].id)
//            t.$Modal.success({
