...
<template>
  <div style="width: 100%;">
    <div id="MonthCompanyOrderPrice" ref="MonthCompanyOrderPrice" style="width: 100%;height: 400px"></div>
  </div>
</template>
<script>
...
//        })
      ...
      getDataPieNum() {
        const t = this
        getDataLevelUserLogin({
          _mt: 'ansrptWorkorderChat.getCostByLastMon',
          logType: '返回最近6个月中工单费用最多的5家公司',
          userId: t.$store.state.user.userId,
          companyId: pubsource.companyId,
        }).then((res) => {
          if (isSuccess(res, t)) {
...
//            console.log(this.echatsName, '公司名称')
//            console.log(this.echatsMonth, '月份')
      ...
      getEchatsColumn(name, month, Obj) {
        let echats = this.$echarts.init(document.getElementById('MonthCompanyOrderPrice'))
        echats.setOption({
          tooltip: {
            trigger: 'axis',
          },
