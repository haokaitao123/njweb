<template>
  <div class="cover">
    <div  class="box">
      ...
      <Col span="24">
      <!-- table表格 日历-->
      <div v-show="calendarModel1" style="" class="calendar_table" >
        ...
        <Row style="min-width: 1200px;">
          ...
          <Row class="table_body_box">
            ...
            <div v-else-if="days.calendarComptype === '01workday' || days.calendarLegaltype === '01workday'" class="conbox area bgWhite">
              ...
              <span v-html="days.taskName" style="white-space:nowrap;display:block;width:95px;text-overflow: ellipsis;white-space: nowrap;overflow:hidden">111</span>
      ...
      <!-- 日历 结束 -->
      <div style="vertical-align: middle;font-size: 14px;padding-top: 10px;margin-top: 10px;border-top-width: 2px; border-top-color: #dddee1; border-top-style: solid">
          {{$t('lang_payroll.paySalplan.salplcfConfirmedDis')}}:
          <RadioGroup :placeholder="$t('lang_payroll.paySalplan.salplcfConfirmedDis')" v-model="confirm">
            <Radio :disabled="exists" label="1" >{{$t('lang_payroll.paySalplan.yes')}}</Radio>
            <Radio :disabled="exists" label="0" >{{$t('lang_payroll.paySalplan.no')}}</Radio>
          </RadioGroup>
          <Button v-show="!exists" style="float: right;" @click="handleReset">{{$t('button.cal')}}</Button>
          <Button v-show="!exists" style="float: right;marginRight: 5px" type="primary" @click="confirmThis">{{$t('button.confirm')}}</Button>
          <Input :disabled="exists" v-if="confirm === '0'" v-model="desc" type="textarea" :autosize="{minRows: 2,maxRows: 5}" :placeholder="$t('lang_payroll.paySalplan.unconfirmed')"></Input>
      </div>
      </Col>
    </div>
    <!-- 进行修改操作弹出的页面 start -->
    <transition name="fade">
      <update v-show="openUpdate" :id="updateId" :logType="logType" :index="index" @closeUp="closeUp" @getData="getData" ref="update"></update>
    </transition>
    <!-- 进行修改操作弹出的页面 end -->

    <transition name="fade">
      <update2 v-show="openUpdate2" :id="updateId" :logType="logType" :index="index" @closeUp="closeUp2" @getData="addNewArray" @update="updateArray" ref="update2"></update2>
    </transition>
    <transition name="fade">
...
//        t.getCaldearData()
      ...
      confirmThis() {
        ...
        if (t.confirm === '') {
          t.$Modal.warning({
            title: this.$t('reminder.remind'),
            content: this.$t('lang_payroll.paySalplan.confirmNull'),
          })
        } else if (t.confirm === '0' && t.desc === '') {
          t.$Modal.warning({
            title: this.$t('reminder.remind'),
            content: this.$t('lang_payroll.paySalplan.unconfirmed'),
          })
        } else {
          t.$Modal.confirm({
            title: this.$t('reminder.remind'),
            content: this.$t('lang_payroll.paySalplan.confirmRes'),
            onOk: () => {
              getDataLevelUserLoginNew({
                _mt: 'paySalplanconfirm.confirm',
                logType: '确认',
                salplcfPlanid: t.planId,
                salplcfConfirmed: t.confirm,
                salplcfReason: t.desc,
              }).then((res) => {
                if (isSuccess(res, t)) {
                  t.handleReset()
                }
      ...
      getConfirm(id) {
        const t = this
        getDataLevelUserLogin({
          _mt: 'paySalplanconfirm.getByPlanid',
          id: id,
          logType: '根据id获取数据',
        }).then((res) => {
          if (isSuccess(res, t)) {
            const d = res.data.content[0]
            if (d.salplcfConfirmed !== undefined && d.salplcfConfirmed !== '') {
              t.exists = true
              t.confirm = d.salplcfConfirmed
              t.desc = d.salplcfReason
            } else {
              t.exists = false
            }
          }
