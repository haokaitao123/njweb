<template>
  <div class="table">
      ...
      <card>
        ...
        <Row>
          <span @dblclick="dbHCompany">
               <Input v-model="plsimnpCompanyDis" style="width: 130px" @on-enter="getData(1)" icon="search" :readonly="true"  :placeholder="$t('lang_payroll.payMonPeriod.plsimnpCompanyDis')"  @on-click="selectHCompany"/>
          </span>
          <span @dblclick="clearFyperiod">
               <Input v-model="plsimnpPeriodDis" style="width: 130px" @on-enter="getData(1)" icon="search" :readonly="true"  :placeholder="$t('lang_payroll.payMonPeriod.plsimnpPeriodDis')"  @on-click="pickFyperiod"/>
          </span>
          <Select v-model="plsimnpSalaryset" @on-change="getData(1)" style="width: 130px" :placeholder="$t('lang_payroll.payMonPeriod.plsimnpSalarysetDis')">
          ...
          <Dropdown>
            ...
            <DropdownMenu slot="list" >
              <span v-for="(item,index) in Salhdstatus" :key="index" @click="selected(item.paramCode,item.paramInfoCn)">
          ...
          <Button type="primary" @click="expData">{{$t('lang_payroll.payMonPeriod.plsimndownTemplate')}}</Button>
          <Button type="primary" @click="importItem">{{$t('lang_payroll.payMonPeriod.plsimnuploadData')}}</Button>
    ...
    <transition name="fade">
      <update v-show="openUpdate" :id="updateId" :logType="logType" :ItemArray="ItemArray" :Salaryset="Salaryset"  :index="index" @closeUp="closeUp" @getData="getData" ref="update"></update>
    </transition>
    <transition name="fade">
    ...
    <transition name="fade">
      <importItem v-show="openImport" :impid="updateId" :itemData="itemData" :imp_mt="imp_mt"  @closeImport="closeImport" ref="importItem"></importItem>
    </transition>
  </div>
</template>
<script>
  ...
  export default{
    data() {
      return {
        ...
        filekey: '',
        filename: '',
        expDisFields: ['plsimnpCompany,公司', 'plsimnpDept,部门', 'plsimnpEmpworkno,员工工号', 'plsimnpCname,中文名', 'plsimnpEmpgid,唯一标识号'],
        openFyperiod: false,
        Salaryset: [],
        Salhdstatus: [],
...
//            plsimnpPeriod: this.plsimnpPeriod,
          ...
          this.$refs.expwindow.handleSubmit1(this.expDisFields, 'paySalitemMonthnoperiod.export', data)
        }
      },
      closeExp() {
        const t = this
      ...
      importItem() {
        const t = this
        if (t.plsimnpPeriod === '' || t.plsimnpSalaryset === undefined || t.plsimnpSalaryset === '') {
          t.$Modal.warning({
            title: this.$t('reminder.remind'),
