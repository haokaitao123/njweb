<template>
  <div class="table">
    <Row>
      ...
      <card>
        <p slot="title">
          ...
          &nbsp;{{$t('lang_payroll.payMonPeriod.title3')}}
        </p>
        <Row>
         <span @dblclick="dbHCompany">
          ...
          <Button type="primary" v-show="plsimnpStatus === '20toupd' ? true:false" @click="handSubmit">{{$t('button.subm')}}</Button>
        </Row>
    ...
    <transition name="fade">
      <update v-show="openUpdate" :id="updateId" :logType="logType" :Salaryset="Salaryset" :ItemArray="ItemArray"  :index="index" @closeUp="closeUp"
              @getData="getData" ref="update"></update>
    </transition>
    <transition name="fade">
  ...
  export default{
    data() {
      return {
        ...
        Salaryset: [],
        Salhdstatus: [],
        select: this.$t('status.waitupdate'),
    ...
    methods: {
      getData(page) {
        ...
        if (data.plsimnpSalaryset === undefined || data.plsimnpSalaryset === '') {
          t.$Modal.warning({
            ...
            content: this.$t('lang_payroll.payMonPeriod.plsimnpReminder2'),
          })
        } else {
      ...
      submit() {
        ...
        const data = {
          ...
          funId: '1',
          logType: '月度非周期性薪资数据hr系统更新',
          id: t.tableselected.toString(),
          plsimnpStatus: '30updated',
        }
        getDataLevelUserLoginNew(data).then((res) => {
          t.tableselected = []
          if (isSuccess(res, t)) {
      ...
      getSelect() {
        ...
        getDataLevelUserLogin({
          ...
          if (isSuccess(res, t)) {
            t.Salhdstatus = res.data.content[0].value[0].paramList.slice(2)
            t.Salhdstatus[0].paramInfoName = '待更新'
          }
        }).catch(() => {
          this.$Modal.error({
            title: this.$t('reminder.err'),
