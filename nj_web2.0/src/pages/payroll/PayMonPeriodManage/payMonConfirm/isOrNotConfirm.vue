    ...
    <div class="box">
      <div class="title">
        <div class="title-text">
          <Icon type="mouse" size="16" style="margin-right: 10px;"></Icon>&nbsp;{{$t('lang_payroll.payMonPeriod.confirmTitle')}}
        </div>
      ...
      <Form ref="form" :model="form" :rules="ruleValidate" :label-width="100">
        <Row>
          ...
          <FormItem :label="$t('lang_payroll.payMonPeriod.plsimnpIsconfirm')" prop="isConfirm">
            <RadioGroup v-model="form.isConfirm">
              <Radio label="1">{{$t('lang_payroll.payMonPeriod.yes')}}</Radio>
              <Radio label="0">{{$t('lang_payroll.payMonPeriod.no')}}</Radio>
            </RadioGroup>
          </FormItem>
          </Col>
        </Row>
        <Row>
          ...
          <FormItem :label="$t('lang_payroll.payMonPeriod.plsimnpNocmreason')" prop="noreason">
            <Input v-model="form.noreason" type="textarea" :autosize="{minRows: 2,maxRows: 2}" :placeholder="$t('lang_payroll.payMonPeriod.plsimnpNocmreasonDis')"></Input>
          </FormItem>
          </Col>
        </Row>
        <Row>
        ...
        this.$refs.form.validate((valid) => {
          if (valid) {
            getDataLevelUserLogin({
              _mt: 'paySalitemMonthnoperiod.updConfirmByIds',
              funId: '1',
