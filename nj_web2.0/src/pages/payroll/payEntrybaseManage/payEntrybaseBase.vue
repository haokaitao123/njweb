            ...
            <row>
                <i-col class="meau-left"
                    ...
                    <Menu :active-name="active"
                        ...
                        <MenuItem name="payPerfmance"
                                  :disabled="logType==='新增'"
                                  @click.native="logType==='新增'?tips():''">业绩信息详情</MenuItem>
                    </Menu>
                </i-col>
                <i-col class="meau-right"
                    ...
                    <div style="margin-top: 40px;padding: 10px;">
                        ...
                        <mOption v-show="option"
                         ...
                         <!--子表分页页面 mainid为主表id-->
                        <payPerfmance v-show="payPerfmance"
                                      ref="payPerfmance"></payPerfmance>
                    </div>
                </i-col>
            </row>
        </div>
...
import mOption from "./payEntrybaseEdit";
import payPerfmance from "./payPerfmance/payPerfmanceList";
export default {
    data () {
        return {
            //初试默认选中
    ...
    methods: {
        ...
        pageTo (name) {
            ...
            this.option = false;
            this.payPerfmance = false;
            this.$refs.option.clear();
            this.$refs.payPerfmance.clear();
            this.active = name;
            this[name] = true;
            if (name !== "option") {
        ...
        clear () {
            ...
            this.$store.commit('payEntrybase/setMainId', "")
            this.$store.commit('payEntrybase/setLogType', "");
        },
        // 修改数据更新父页面列表 
        update (data) {
