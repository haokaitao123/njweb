      ...
      <row>
        ...
        <i-col class="meau-right" span="19">
          ...
          <div style="margin-top: 40px;padding: 10px;">
            ...
            ></mOption>
            <!--子表分页页面 mainid为主表id-->
            <payDrawdetail v-show="payDrawdetail" ref="payDrawdetail"></payDrawdetail>
          </div>
        </i-col>
      </row>
    </div>
...
import mOption from "./payLeavepropEdit";
import payDrawdetail from "./payDrawdetail/payDrawdetailList";
export default {
  data() {
    return {
      //初试默认选中
  ...
  methods: {
    ...
    pageTo(name) {
      ...
      this.option = false;
      this.payDrawdetail = false;
      this.$refs.option.clear();
      this.$refs.payDrawdetail.clear();
      this.active = name;
      this[name] = true;
      if (name !== "option") {
    ...
    clear() {
      ...
      this.$store.commit("payLeaveprop/setMainId", "");
      this.$store.commit("payLeaveprop/setLogType", "");
    },
    // 修改数据更新父页面列表
    update(data) {
