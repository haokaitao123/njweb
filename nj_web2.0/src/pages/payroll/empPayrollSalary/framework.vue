        ...
        <i-col class="meau-left" span="5">
          <Menu :active-name="active" width="auto" @on-select="pageTo">
            <MenuItem name="option">
              {{$t('lang_payroll.salaryBase.payrollBase')}}
            </MenuItem>
            <MenuItem name="content" v-show="id">
              {{$t('lang_payroll.salaryBase.payrollItem')}}
            </MenuItem>
          </Menu>
        </i-col>
...
<script>
  import mOption from './salaryBaseView'
  import mContent from './salaryItemManage'
  ...
  export default {
    data() {
      return {
        ...
        id: NaN,
        data: {
          _mt: 'empPayrollSalaryBase.getById',
        },
      }
    },
    components: {
