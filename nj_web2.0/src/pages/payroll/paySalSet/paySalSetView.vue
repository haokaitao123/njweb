<template>
  <div class="option-main">
    <Row style="max-height: 420px;overflow-y: auto;">
      <Form :model="form" label-position="right" :rules="ruleValidate" ref="form" :label-width="120">
        ...
        <FormItem :label="$t('lang_payroll.paySalSet.payssCode')"  prop="payssCode">
          <Input v-model="form.payssCode" :placeholder="$t('lang_payroll.paySalSet.payssCodeDis')" ></Input>
        </FormItem>
      </i-col>
        ...
        <i-col span="11" offset="1">
          <FormItem :label="$t('lang_payroll.paySalSet.payssLocalcurrency')" prop="payssLocalcurrency">
          <span @dblclick="clearCurrency">
            <Input v-model="payssLocalcurrencyDis" icon="search" readonly :placeholder="$t('lang_payroll.paySalSet.payssLocalcurrencyDis')" style="width: 200px" @on-click="pickCurrency" />
          </span>
          </FormItem>
        </i-col>
        <i-col span="11">
          <FormItem :label="$t('lang_payroll.paySalSet.payssContact')" prop="payssContact">
            <Input v-model="form.payssContact" :placeholder="$t('lang_payroll.paySalSet.payssContactDis')" ></Input>
          </FormItem>
        </i-col>
        <i-col span="11" offset="1">
          <FormItem :label="$t('lang_payroll.paySalSet.payssEmail')" prop="payssEmail">
            <Input v-model="form.payssEmail" :placeholder="$t('lang_payroll.paySalSet.payssEmailDis')" ></Input>
          </FormItem>
        </i-col>
        <i-col span="11">
          <FormItem :label="$t('lang_payroll.paySalSet.payssPhone')" prop="payssPhone">
            <Input v-model="form.payssPhone" :placeholder="$t('lang_payroll.paySalSet.payssPhoneDis')" ></Input>
          </FormItem>
        </i-col>
        <i-col span="23">
        <FormItem :label="$t('lang_payroll.paySalSet.payssComment')" prop="payssComment">
          <Input v-model="form.payssComment" type="textarea" :autosize="{minRows: 2,maxRows: 5}" :placeholder="$t('lang_payroll.paySalSet.payssCommentDis')"></Input>
        </FormItem>
        </i-col>
      </Form>
    </Row>
    ...
    <transition name="fade">
      <searchCurrency v-show="openCurrency" :params="params" :searchCloumns="searchCloumns" @closeUp="closeCurrency" @changeinput="changeinput" ref="searchCurrency"></searchCurrency>
    </transition>
  </div>
</template>
<script>
  ...
  export default {
    data() {
      return {
        ...
        ruleValidate: {
          payssCode: [
            { required: true, message: this.$t('lang_payroll.paySalSet.payssCodeDis'), trigger: 'blur' },
          ],
          payssCN: [
            { required: true, message: this.$t('lang_payroll.paySalSet.payssCNDis'), trigger: 'blur' },
          ],
          payssEN: [
            { required: true, message: this.$t('lang_payroll.paySalSet.payssENDis'), trigger: 'blur' },
          ],
          payssLocalcurrency: [
            { required: true, message: this.$t('lang_payroll.paySalSet.payssLocalcurrencyDis'), trigger: 'blur' },
          ],
        },
        openCurrency: false,
        params: {
      ...
      getdata() {
        ...
        getDataLevelUserLogin(params).then((res) => {
          if (isSuccess(res, t)) {
            ...
            this.payssLocalcurrencyDis = res.data.content[0].payssLocalcurrencyDis
          }
        }).catch(() => {
          t.$Modal.error({
        ...
        getDataLevelUserLoginNew(data).then((res) => {
          if (isSuccess(res, t)) {
            ...
            if (t.logType === this.$t('button.add')) {
              ...
              t.$store.commit('paySalSet/setChildId1', res.data.content[0].id)
