<template>
  <div class="cover">
        ...
        <Form :model="form" ref="form" label-position="right" :rules="ruleValidate" :label-width="120">
          ...
          <i-col span="11" >
            <FormItem :label="$t('lang_payroll.paySalSetItem.payssiOrder')" prop="payssiOrder">
              <Input v-model="form.payssiOrder" :placeholder="$t('lang_payroll.paySalSetItem.payssiOrderDis')" ></Input>
            </FormItem>
          </i-col>
          <i-col span="11"  >
            ...
            <!--<span @dblclick="clearCityid">-->
              <!-- @on-click="pickData" -->
              <Input v-model="payssiItemName" icon="search" :readonly="true" :placeholder="$t('lang_payroll.paySalSetItem.payssiItemNameDis')" disabled="disabled" />
            <!--</span>-->
            </FormItem>
          </i-col>
          <i-col span="11">
            <FormItem :label="$t('lang_payroll.paySalSetItem.payssiItemtypeName')" prop="payssiItemtype">
              <Select v-model="form.payssiItemtype" disabled :placeholder="$t('lang_payroll.paySalSetItem.payssiItemtypeNameDis')">
                <Option :value="item.paramCode" v-for="(item,index) in selectItemType" :key="index">{{item.paramInfoCn}}</Option>
              </Select>
            </FormItem>
          </i-col>
          <i-col span="11" >
            <FormItem :label="$t('lang_payroll.paySalSetItem.payssiDatasource')" prop="payssiDatasource">
              <Select v-model="form.payssiDatasource" clearable  :placeholder="$t('lang_payroll.paySalSetItem.payssiDatasourceDis')">
          ...
          <i-col span="23" v-if="form.payssiDatasource === '05formula'" >
            <FormItem :label="$t('lang_payroll.paySalSetItem.payssiFormula')" prop="payssiFormula">
              <Input v-model="form.payssiFormula"  type="textarea" disabled :autosize="{minRows: 2,maxRows: 5}" :placeholder="$t('lang_payroll.paySalSetItem.payssiFormulaDis')" ></Input>
            </FormItem>
          </i-col>
          <i-col span="23" v-if="form.payssiDatasource === '05formula'" >
            <FormItem :label="$t('lang_payroll.paySalSetItem.payssiFormulacomment')" prop="payssiFormulacomment">
              <Input v-model="form.payssiFormulacomment" type="textarea" :autosize="{minRows: 2,maxRows: 5}" :placeholder="$t('lang_payroll.paySalSetItem.payssiFormulacommentDis')"></Input>
            </FormItem>
          </i-col>
          <i-col span="11">
            <FormItem :label="$t('lang_payroll.paySalSetItem.payssiCarrymode')" prop="payssiCarrymode">
              <Select v-model="form.payssiCarrymode" clearable  :placeholder="$t('lang_payroll.paySalSetItem.payssiCarrymodeDis')">
                <Option :value="item.paramCode" v-for="(item,index) in DecimalRule" :key="index">{{item.paramInfoCn}}</Option>
              </Select>
            </FormItem>
          </i-col>
          <i-col span="11" >
            <FormItem :label="$t('lang_payroll.paySalSetItem.payssiPaylistdis')" prop="payssiPaylistdis">
              <RadioGroup v-model="form.payssiPaylistdis" :placeholder="$t('lang_payroll.paySalSetItem.payssiSelectDis')">
          ...
          <i-col span="11" v-show="form.payssiItemtype === '01sysperiodic' || form.payssiItemtype === '03userperiodic'?true:false">
            <FormItem :label="$t('lang_payroll.paySalSetItem.payssiObversion')" prop="payssiObversion">
              <RadioGroup v-model="form.payssiObversion" :placeholder="$t('lang_payroll.paySalSetItem.payssiSelectDis')">
                <Radio :label="item.paramCode" v-for="(item,index) in YesOrNoType" :key="index">{{item.paramInfoCn}}</Radio>
              </RadioGroup>
          ...
          <i-col span="11" >
            <FormItem :label="$t('lang_payroll.paySalSetItem.isValid')" prop="isValid">
              <RadioGroup v-model="form.isValid" :placeholder="$t('lang_payroll.paySalSetItem.payssiSelectDis')">
                <Radio :label="item.paramCode" v-for="(item,index) in YesOrNoType" :key="index">{{item.paramInfoCn}}</Radio>
              </RadioGroup>
          ...
          <i-col span="23">
            <FormItem :label="$t('lang_payroll.paySalSetItem.payssiComment')" prop="payssiComment">
              <Input v-model="form.payssiComment" type="textarea" :autosize="{minRows: 2,maxRows: 5}" :placeholder="$t('lang_payroll.paySalSetItem.payssiCommentDis')"></Input>
            </FormItem>
          </i-col>
          <i-col span="23">
    ...
    <transition name="fade">
      <searchProllItem v-show="openPick" :searchCloumns="searchCloumns" :params="params" @closeUp="close1" @changeinput="changeinput" ref="searchProllItem"></searchProllItem>
    </transition>
  </div>
</template>
<script>
  ...
  export default {
    data() {
      return {
        ...
        ruleValidate: {
          payssiOrder: [
            { required: true, message: this.$t('lang_payroll.paySalSetItem.payssiOrderDis'), trigger: 'blur' },
          ],
          payssiItem: [
            { required: true, message: this.$t('lang_payroll.paySalSetItem.payssiItemNameDis'), trigger: 'blur' },
          ],
          payssiItemtype: [
            { required: true, message: this.$t('lang_payroll.paySalSetItem.payssiItemtypeDis'), trigger: 'blur' },
          ],
          payssiDatasource: [
            { required: true, message: this.$t('lang_payroll.paySalSetItem.payssiDatasourceDis'), trigger: 'blur' },
          ],
//          payssiCarrymode: [
      ...
      getdata(rowId) {
        ...
        getDataLevelUserLogin(params).then((res) => {
          if (isSuccess(res, t)) {
            ...
            t.payssiItemName = res.data.content[0].payssiItemName
            t.setDataSouType(res.data.content[0].payssiItemtype)
            t.form.isValid = '1'
          }
        }).catch(() => {
          t.$Modal.error({
