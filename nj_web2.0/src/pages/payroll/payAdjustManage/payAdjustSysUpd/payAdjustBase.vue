      ...
      <row>
        <i-col class="meau-left" span="5">
          <Menu :active-name="active" width="auto" @on-select="pageTo">
            <MenuItem name="option">
              {{$t('lang_payroll.payAdjust.baseTitle')}}
            </MenuItem>
            <MenuItem name="content" v-show="id">
              {{$t('lang_payroll.payAdjust.baseSubTitle')}}
            </MenuItem>
          </Menu>
        </i-col>
        <i-col class="meau-right" span="19">
          ...
          <div style="margin-top: 40px;padding: 10px;">
            <mOption v-show="option" :logType="logType" ref="option" :id="id" @getid="getid" @handleReset="handleReset" @update="update" @newdata="newdata"></mOption>
...
<script>
  import mOption from './updPayAdjust'
