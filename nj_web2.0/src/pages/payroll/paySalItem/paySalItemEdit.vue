    ...
    <div class="box">
      ...
      <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="135">
        <Row>
          <Col span="10" offset="1">
            <FormItem :label="$t('lang_payroll.paySalItem.paysiOrder')" prop="paysiOrder">
              <Input
                v-model="formValidate.paysiOrder"
                :placeholder="$t('lang_payroll.paySalItem.paysiOrderDis')"
              ></Input>
            </FormItem>
          </Col>
          <Col span="10" offset="1">
            <FormItem :label="$t('lang_payroll.paySalItem.paysiCode')" prop="paysiCode">
              <Input
                v-model="formValidate.paysiCode"
        ...
        <Row>
          <Col span="10" offset="1">
            <FormItem
              :label="$t('lang_payroll.paySalItem.paysiDefaulttypeName')"
              prop="paysiDefaulttype"
            >
              <Select
                @on-change="setDataSouType"
                v-model="formValidate.paysiDefaulttype"
                :placeholder="$t('lang_payroll.paySalItem.paysiDefaulttypeNameDis')"
              >
                <Option
          ...
          <Col span="10" offset="1">
            <FormItem :label="$t('lang_payroll.paySalItem.paysiCN')" prop="paysiCN">
              <Input
                v-model="formValidate.paysiCN"
                :placeholder="$t('lang_payroll.paySalItem.paysiCNDis')"
              ></Input>
            </FormItem>
          </Col>
        </Row>
        <Row>
          <Col span="10" offset="1">
            <FormItem :label="$t('lang_payroll.paySalItem.paysiEN')" prop="paysiEN">
              <Input
                v-model="formValidate.paysiEN"
                :placeholder="$t('lang_payroll.paySalItem.paysiENDis')"
              ></Input>
            </FormItem>
          </Col>
          <Col span="10" offset="1">
            <FormItem
              :label="$t('lang_payroll.paySalItem.paysiIsdefaultssiName')"
              prop="paysiIsdefaultssi"
            >
              <RadioGroup v-model="formValidate.paysiIsdefaultssi" placeholder="请选择是否确认项目">
                <Radio
                  :label="item.paramCode"
                  v-for="(item,index) in selectIsType"
                  :key="index"
        ...
        <Row>
          <Col span="10" offset="1">
            <FormItem
              :label="$t('lang_payroll.paySalItem.paysiDefaultdtsourceName')"
              prop="paysiDefaultdtsource"
            >
              <Select
                v-model="formValidate.paysiDefaultdtsource"
                :placeholder="$t('lang_payroll.paySalItem.paysiDefaultdtsourceNameDis')"
              >
                <Option
                  :value="item.paramCode"
        ...
        <Row>
          <Col span="21" offset="1">
            <FormItem :label="$t('lang_payroll.paySalItem.paysiComment')" prop="paysiComment">
              <Input
                v-model="formValidate.paysiComment"
                type="textarea"
                :autosize="{minRows: 2,maxRows: 5}"
                :placeholder="$t('lang_payroll.paySalItem.paysiCommentDis')"
              ></Input>
            </FormItem>
          </Col>
        </Row>
    ...
    return {
      ...
      ruleValidate: {
        paysiOrder: [
          {
            required: true,
            message: this.$t("lang_payroll.paySalItem.paysiOrderDis"),
            trigger: "blur"
          }
        ],
        paysiCode: [
          {
            required: true,
            message: this.$t("lang_payroll.paySalItem.paysiCodeDis"),
            trigger: "blur"
          }
        ],
        paysiDefaulttype: [
          {
            required: true,
            message: this.$t("lang_payroll.paySalItem.paysiDefaulttypeNameDis"),
            trigger: "blur"
          }
        ],
        paysiCN: [
          {
            required: true,
            message: this.$t("lang_payroll.paySalItem.paysiCNDis"),
            trigger: "blur"
          }
        ],
        paysiEN: [
          {
            required: true,
            message: this.$t("lang_payroll.paySalItem.paysiENDis"),
            trigger: "blur"
          }
        ],
        paysiIsdefaultssi: [
          {
            required: true,
            message: this.$t(
              "lang_payroll.paySalItem.paysiIsdefaultssiNameDis"
            ),
            trigger: "blur"
          }
        ],
        paysiDefaultdtsource: [
          {
            required: true,
            message: this.$t(
              "lang_payroll.paySalItem.paysiDefaultdtsourceNameDis"
            ),
            trigger: "blur"
          }
        ]
        ...
        .then(res => {
          ...
          res = {
            stat: {
              code: 0,
              stateList: [
                { msg: "SUCCESS", code: 0, length: 478, desc: "成功" }
              ],
              systime: 1561621707912,
              desc: "成功",
              cid: "a:18bb9c|t:153602|s:1561621707858"
            },
            content: [
              {
                paysiComment: "",
          ...
          //}
          this.formValidate = res.content[0];
        })
        .catch(() => {
          this.$Modal.error({
