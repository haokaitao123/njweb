...
const emp_flow_dimission = {
  ...
  empdimCompany(node) {
    emp_flow_dimission.empdimCompany_set(this.$parent)
  },
  empdimDept(node) {
    emp_flow_dimission.empdimDept_set(this.$parent)
  },
  empdimDept_set(t) {
    let com
    let comDis
    if (t.valueMap.empdimDept) {
      com = t.$refs[t.valueMap.empdimDept][0].formDataSubmit.empdimDept
      comDis = t.$refs[t.valueMap.empdimDept][0].$refs.empdimDept.thisValue
    }

    t.$refs[t.valueMap.empdimWorkno][0].$set(t.$refs[t.valueMap.empdimWorkno][0].formDataSubmit, 'empdimWorkno', '')
    t.$refs[t.valueMap.empdimWorkno][0].$refs.empdimWorkno.thisValue = ''
    t.$store.state.flowClmkMap.popForm.empdimWorkno = ''  // 弹出框，清空存放的值
  },
  empdimCompany_set(t) {
    let com
    let comDis
    if (t.valueMap.empdimCompany) {
      com = t.$refs[t.valueMap.empdimCompany][0].formDataSubmit.empdimCompany
      comDis = t.$refs[t.valueMap.empdimCompany][0].$refs.empdimCompany.thisValue
    }
    t.$refs[t.valueMap.empdimDept][0].$set(t.$refs[t.valueMap.empdimDept][0].formDataSubmit, 'empdimDept', '')
    t.$refs[t.valueMap.empdimDept][0].$refs.empdimDept.thisValue = ''
    t.$store.state.flowClmkMap.popForm.empdimDept = ''  // 弹出框，清空存放的值
