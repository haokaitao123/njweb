    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="mouse" size="16" style="margin-right: 10px;"></Icon>&nbsp;表格
        </div>
      ...
      <Row class="table-form">
        ...
        <span style="margin: 0;"><Button type="primary" icon="search" @click="getData(param, 1)">{{$t('button.ser')}}</Button></span>
      </Row>
      <row class="table-form" ref="table-form">
        <Table height="280" size="small" border ref="selection" :columns="searchCloumns" @on-sort-change="sortable" :data="data" @on-row-dblclick="dbCkick"></Table>
      </row>
      <Row style="display: flex">          <Page :total="total" showTotal size="small" show-elevator show-sizer :current="param.page" placement="top" @on-page-size-change="sizeChange" @on-change="pageChange":page-size-opts = "[10, 20, 50, 100]" ></Page><Button type="ghost" size="small" shape="circle" icon="refresh" style="margin-left: 20px;display: inline-block;" @click="getData(1)"></Button></Row>
    </div>
  </div>
</template>
<script>
  import searchTable from './searchTable'
  import { getDataLevelUserLoginNewId } from '../../../axios/axios'
  ...
  export default{
    ...
    methods: {
      ...
      getData(param, page) {
        ...
        if (rule) {
          for (let i = 0; i < rule.length; i++) {
            if (this.$store.state.flowClmkMap.popForm[rule[i].value]) {
              console.log(rule[i].key, 'rule[i].key')
              console.log(this.$store.state.flowClmkMap.popForm[rule[i].value], 'this.$store.state.flowClmkMap')
        ...
        getDataLevelUserLoginNewId(data).then((res) => {
          if (isSuccess(res, t)) {
      ...
      dbCkick(row) {
        ...
        for (let i = 0; i < aa.length; i++) {
          let key = aa[i].key
          let value = aa[i].value
          let vI = row[aa[i].value]
