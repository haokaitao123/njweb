    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="mouse" size="16" style="margin-right: 10px;"></Icon>&nbsp;诊断对象
        </div>
      ...
      <Row class="table-form">
        <Input placeholder="请输入公司编码" style="width: 200px" v-model="pmtypeCode"/>
      ...
      <Row style="display: flex">          <Page :total="total" showTotal :current="current" size="small" show-elevator show-sizer placement="top" @on-page-size-change="sizeChange" @on-change="pageChange" :page-size=[rows] :page-size-opts = "[10, 20, 50, 100]" ></Page><Button type="ghost" size="small" shape="circle" icon="refresh" style="margin-left: 20px;display: inline-block;" @click="getData(1)"></Button></Row>
    </div>
  </div>
</template>
<script>
  ...
  export default{
    ...
    methods: {
      ...
      dbCkick(row) {
        const t = this
        this.$emit('changeinputDiag',row.id, row.resEmpid ,row.resEmpidDis,t.num)
        t.close()
      },
      sizeChange(size) {
        const t = this
