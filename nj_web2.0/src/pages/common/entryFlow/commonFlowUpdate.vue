              ...
              <row class="table-form" ref="table-form" :key="indexs" v-for="(items,indexs) in columnsChildAll" v-if="!item.blockColumn && item.id == items[0].dataBlockId">
                <row v-for="(itemTable,indexTable) in dataTable" :key="indexTable" v-if="!item.blockColumn && item.id == items[0].dataBlockId">
                  ...
                  <span v-if="!item.blockColumn && item.id == itemTable.dataBlockId" @click="PageSize(itemTable.dataBlockId)">
                    <Row style="display: flex">          <Page :total="totalTable" :current="itemTable.page" size="small" show-elevator  placement="top"  @on-page-size-change="sizeChange" @on-change="pageChange" :page-size-opts = "[10, 20, 50, 100]" ></Page><Button type="ghost" size="small" shape="circle" icon="refresh" style="margin-left: 20px;display: inline-block;" @click="getData(1)"></Button></Row>
                  </span>
                </row>
              </row>
            </div>
...
//        this.getPageChildTable(this.dataBlocksFakeId[1])
//        this.getPageChildTable(this.dataBlocksFakeId[2])
      })
      this.getColumns()
//      this.getDataBlock()
    },
...
//          this.logType = this.$t('button.upd')
...
//          console.log(t.dataTable)
      ...
      getColumnChildTable(dataBlockId) {
        ...
        getDataLevelUserLogin(params).then(res => {
          ...
          t.columnsChild = aa
//          if (t.revise.dataBlockId !== t.columnsChild[0].dataBlockId) {
//            t.revise.dataBlockId = t.columnsChild[0].dataBlockId
//            t.$set(t.revise, 'dataBlockId', t.columnsChild[0].dataBlockId)
//          }
          t.columnsChild.push(t.revise)
//          for (let m = 0; m < t.ChildDataBloks.length; m++) {
//            if (t.ChildDataBloks[m].id === t.columnsChild[0].dataBlockId) {
//              if (t.ChildDataBloks[m].flsdbOptauth !== '02update' && this.stepAuthLimits !== '03submit' && t.ChildDataBloks[m].flsdbSubisupd !== '1') {
//                t.columnsChild[1].dataBlockId = t.columnsChild[0].dataBlockId
//                t.columnsChild.splice(0, 1)
//              }
//            }
//          }
          t.columnsChildAll.push(t.columnsChild)
//          for (let m = 0; m < t.columnsChildAll.length; m++) {
//            this.$set(t.columnsChildAll[m][t.columnsChildAll.length - 1], 'dataBlockId', t.columnsChildAll[m][0].dataBlockId)
//          }
...
//          console.log(t.columnsChildAll)
//          console.log(t.revise)
        }).catch(err => {
          console.log(err)
        })
      },
