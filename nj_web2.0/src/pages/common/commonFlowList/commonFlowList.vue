<template>
	<div class="table">
		<Row>
			...
			<card>
					...
					<div class="moditySelect">
						<Dropdown>
							...
							<DropdownMenu slot="list">
								<span v-for="(item,index) in dropdownMenuList" :key="index" @click="getPageByState(item.id,item.flstepName)">
				...
				<row class="table-form" ref="table-form">
					<Table @on-selection-change="selectedtable" border :height="tableheight" size="small" ref="selection" :columns="columns" :loading="loading" :data="data"></Table>
				</row>
				<Row style="display: flex">
					<Page :total="total" size="small" :current="page" show-elevator show-sizer placement="top" @on-page-size-change="sizeChange" @on-change="pageChange" :page-size=rows :page-size-opts="[10, 20, 50, 100]"></Page>
		...
		<transition name="fade">
			<transaction v-show="openTransaction" :id="transactionId" :logType="logType" @closeTransaction="closeTransaction" ref="transactionWindow"></transaction>
		</transition>
		<commonFlowUpdate v-if="openTestUpd" @close="closeTest" ref="commonFlowUpdate" @getData="getData" :columns="columns" :flowId="flowId" :pkValue="pkValue" :stepId="stepId" :setpName="stepName" :stepState="stepState" :processState="processState">
		</commonFlowUpdate>
	</div>
</template>
<script>
