<template>
  <div class="table">
    <Row>
      <Col span="24">
        <card>
          ...
          <Row>
            <Input
              :placeholder="tbName=='recruit_process' ? '请输入求职者姓名' : '请输入员工姓名'"
              style="width: 200px"
              @on-enter="enterEvent"
    ...
    <loading v-show="showLoading" @closeLoading="closeLoading" ref="wfloading"></loading>
    <transition name="fade">
      <selCountry
        v-show="openSelCountry"
        selSort="id"
        selOrder="asc"
        @selOk="countrySelOk"
    ...
    <transition name="fade">
      <transaction
        v-show="openTransaction"
        :id="transactionId"
        :logType="logType"
        @closeTransaction="closeTransaction"
        ref="transactionWindow"
      ></transaction>
    </transition>
    <transition name="fade">
      <interviewOrder
        v-show="openOrdersaction"
        :id="exportselected"
        :logType="logType"
...
export default {
  ...
  methods: {
                        ...
                        h(
                          "div",
                          {
                            ...
                            on: {
                              click: async () => {
                                let stepId =
                                  t.flowStep[params.column.key].stepid;
                                t.stepState =
                                  params.row.cellClassName[aa[i].key];
                                t.processState =
                                  t.flowStep[params.column.key].handleStatus;
                                if (t.stepState === "p_flowst_0") {
                                  return;
                                }
                                t.stepName =
                                  t.flowStep[params.column.key].flstepName;
                                if (
                                  t.stepName === "复试" &&
                                  (t.stepState === "p_flowst_1" ||
    ...
    btnFunction(btnId) {
      ...
      if (btnId === "button_export") {
        ...
        this.$refs.expwindow.getData(
          t.getexpDataTital(t.tbName),
          "platAutoLayoutExport.export",
          data
        );
        this.openExp = true;
      }
    ...
    getexpDataTital(tbName) {
      ...
      if (tbName === "emp_empdim") {
        ...
        expDataTital = [
          ...
          { code: "goutBgdate", name: "外出开始日期" },
          { code: "goutEddate", name: "外出结束日期" },
          { code: "note", name: "备注" }
        ];
        return expDataTital;
    ...
    getData(page) {
      ...
      t.relibFilldate =
        t.relibFilldate != ""
          ? new Date(t.relibFilldate).format("yyyy-MM-dd")
          : "";
      t.relibReexamtm =
        t.relibReexamtm != ""
          ? new Date(t.relibReexamtm).format("yyyy-MM-dd")
          : "";
      t.dimLevsqday =
        t.dimLevsqday != "" ? new Date(t.dimLevsqday).format("yyyy-MM-dd") : "";
      const rcdata = {
        curStep: t.curStep,
        flowId: t.flowId,
        empnhName: t.empnhName,
