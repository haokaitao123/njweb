    ...
    <Row>
      ...
      <card>
        <p slot="title">
          ...
          &nbsp;{{$t('lang_communication.archborFlow.title')}}
        </p>
      <Row>
        <i-col span="24">
          ...
          <span @dblclick="clearUserid">
            {{$t('lang_communication.archborFlow.borBorrower')}}：<Input v-model="borBorrowerDis" icon="search" :readonly="true" :placeholder="$t('lang_communication.archborFlow.borBorrowerDis')" style="width: 120px" @on-click="pickUserData" />
          </span>
      ...
      <Row>
        <i-col span="24">
          ...
          <Row style="display: flex">
            <Page :total="total" size="small" :current="page" show-elevator show-sizer placement="top" @on-page-size-change="sizeChange" @on-change="pageChange":page-size-opts = "[10, 20, 50, 100]" ></Page><Button type="ghost" size="small" shape="circle" icon="refresh" style="margin-left: 20px;display: inline-block;" @click="getData(1)"></Button></Row>
        </i-col>
      </Row>
      </card>
      </Col>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      inputEmp(row) {
        const t = this
        t.borBorrowerDis = row.empName
        t.borBorrower = row.id
      },
      clearUserid() {
        const t = this
        t.borBorrowerDis = ''
      ...
      getData() {
        ...
        data['borBorrower'] = t.borBorrower
        for (const dat in data) {
          if (data[dat] === '') {
            delete data[dat]
          }
