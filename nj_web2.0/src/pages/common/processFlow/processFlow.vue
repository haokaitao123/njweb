    ...
    <Row>
      ...
      <card>
        <p slot="title">
          ...
          &nbsp;{{$t('lang_communication.processFlow.title')}}
        </p>
      <Row>
        <i-col span="24">
          ...
          {{$t('lang_communication.processFlow.payproSalset')}}:
          <Select style="width: 100px"  v-model="payproSalset"  :placeholder="$t('lang_communication.processFlow.payproSalsetDis')">
            <Option :value="item.id"  v-for="(item,index) in selectSalset" :key="index">{{item.payss}}</Option>
          </Select>
          <span @dblclick="dbHCompany">
               {{$t('lang_communication.processFlow.period')}}: <Input v-model="period" style="width: 100px" icon="search" :readonly="true" :placeholder="$t('lang_communication.processFlow.periodDis')"
                                         @on-click="pickFyperiod"/>
          </span>
          <Select style="width: 100px"  v-model="payproRound"  :placeholder="$t('lang_communication.processFlow.payproRoundDis')">
            <Option :value="item.paramCode"  v-for="(item,index) in selectRound" :key="index">{{item.paramInfoCn}}</Option>
          </Select>
...
//      },
      ...
      getData() {
        ...
        data['payproSalset'] = t.payproSalset
        data['payproPeriod'] = t.payproPeriod
        data['payproRound'] = t.payproRound
...
//	        	this.getCommunRecordData(pRow.id)
      ...
      businessLook(pRow) {
        ...
        getDataLevelUserLoginId({
          ...
          t.openId = res.data.content[0].value.split('_')[2]
      ...
      getSelect() {
        ...
        getDataLevelUserLogin({
          ...
          if (isSuccess(res, t)) {
            t.selectRound = res.data.content[0].value[0].paramList
          }
        }).catch(() => {
          this.$Modal.error({
            title: this.$t('reminder.err'),
