    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="mouse" size="16" style="margin-right: 10px;"></Icon>&nbsp;{{$t('lang_personalfile.fileQuery.searchTitle')}}
        </div>
      ...
      <Row class="table-form">
        <Input :placeholder="$t('lang_personalfile.fileQuery.claNameDis')" style="width: 200px" v-model="claName"/>
  ...
  export default{
    ...
    methods: {
      ...
      dbCkick(row) {
        const t = this
        console.log(row)
        this.$emit('changeinput', row.id, row.claName)
        t.close()
      },
      sizeChange(size) {
        const t = this
