    ...
    <Row>
      ...
      <card>
        <p slot="title">
          ...
          &nbsp;{{$t('lang_knowBase.knowlegebase.title2')}}
        </p>
        <Row>
          ...
          <Button type="primary" v-show="cmutKldState == '02confirm' || cmutKldState == '01entry' " @click="submitBtn(1, $t('button.confirm'))">{{ $t('button.confirm')}}</Button>
          <Button type="error" v-show="cmutKldState == '02confirm'" @click="submitBtn(0, $t('button.back'))">{{$t('button.back')}}</Button>
        </Row>
        <!--<Row>-->
          <!--<RadioGroup v-model="paccStatus" v-for="(item,index) in knowledgeStatus" :key="index" style="margin-top: 5px"  @on-change="selected">-->
