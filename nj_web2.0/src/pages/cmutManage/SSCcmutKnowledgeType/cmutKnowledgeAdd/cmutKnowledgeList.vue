<template>
  <div class="table">
    <Row>
      ...
      <card>
        <p slot="title">
          ...
          &nbsp;{{$t('lang_knowBase.knowlegebase.title')}}
        </p>
        <Row>
          ...
          <Button type="primary" @click="submitBtn()">{{$t('button.subm')}}</Button>
        </Row>
    ...
    <transition name="fade">
      <update v-show="openUpdate" :submit="submit" :knowledgeStatus="knowledgeStatus" :Knowauthtype="Knowauthtype" :yesno="yesno" :knowledgeType="knowledgeType" :id="updateId" ref="update" :logType="logType" :index="index"
              @closeUp="closeUp" @getData="addNewArray" @update="updateArray"></update>
    </transition>
  </div>
</template>
<script>
    ...
    methods: {
      ...
      openUp(id, logType, index) {
        ...
        if (logType === this.$t('button.upd')) {
          ...
          t.$refs.update.getPubAuorAndComp() // 获取发布者姓名和发布者绑定员工的雇佣公司
        }
      },
