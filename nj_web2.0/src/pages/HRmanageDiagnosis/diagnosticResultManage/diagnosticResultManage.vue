    ...
    <Row>
      ...
      <card>
        <p slot="title">
          ...
          &nbsp;{{$t('lang_evaluation.evaluation.diagnosticResult')}}
        </p>
        <Row>
          <Col span="20" style="width:100% !important;">
          <Row>
            <Input :placeholder="$t('lang_evaluation.evaluation.resYearIns')" style="width: 200px" v-model="resEmpTimeName" />
            <span @dblclick="dbHCompany">
             <Input v-model="resEmpidDis" style="width: 190px" icon="search" :readonly="true" :placeholder="$t('lang_evaluation.evaluation.resEmpidDisIns1')"  @on-click="selectHCompany"/>
            </span>
  ...
  export default{
    data() {
      return {
        ...
        openImport: false,
        expDataTital: [{ code: 'resEmpidDis', name: '诊断对象' }, { code: 'resYear', name: '诊断年份' },
          { code: 'resProidDis', name: '方案名称'}, { code: 'resStime', name: '诊断开始时间' },
          { code: 'resEtime', name: '诊断结束时间' }],
				resEmpid: '',
        resEmpidDis: '',
        resEmpTimeName: '',
        resEmpTimeid: '',
        ...
        columns: [
          ...
          {
            title: this.$t('lang_evaluation.evaluation.resYear'),
            key: 'resYear',
            sortable: 'custom',
          },
          {
            title: this.$t('lang_evaluation.evaluation.resEmpidDis'),
            key: 'resEmpidDis',
            sortable: 'custom',
          },
          {
          ...
          {
            title: this.$t('lang_evaluation.evaluation.resStime'),
            key: 'resStime',
            sortable: 'custom',
          },
          {
            title: this.$t('lang_evaluation.evaluation.resEtime'),
            key: 'resEtime',
            sortable: 'custom',
          },
          {
            ...
            render: (h, params) => {
              return h('div', [
                ...
                }, this.$t('lang_evaluation.evaluation.diagnosticScoreResult')),
              ])
            },
          },
        ],
