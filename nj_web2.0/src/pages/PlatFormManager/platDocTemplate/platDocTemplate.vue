  ...
  <div class="table">
    <Row>
      ...
      <card>
        ...
        <Row>
          <Input :placeholder="$t('lang_platdoc.platDoc.p_platDctpCode')" style="width: 200px" v-model="platDctpCode"/>
          <Input :placeholder="$t('lang_platdoc.platDoc.p_platDctpName')" style="width: 200px" v-model="platDctpName"/>
          <span style="margin: 0;"><Button type="primary" icon="search" @click="getData(1)">{{$t('button.ser')}}</Button></span>
          <Button type="primary" @click="openUp('',$t('button.add'))">{{$t('button.add')}}</Button>
          <Button type="error" @click="deletemsg">{{$t('button.del')}}</Button>
        </Row>
        <row class="table-form" ref="table-form">
          <Table @on-selection-change="selectedtable" @on-sort-change="sortable" :height="tableheight" size="small" border ref="selection" :columns="columns" :data="$store.state.docTemp.mainTable"></Table>
        </row>
