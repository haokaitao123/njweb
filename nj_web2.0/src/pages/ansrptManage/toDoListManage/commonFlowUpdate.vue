...
export default {
    ...
    methods: {
        ...
        getDataFlowBlock () {
            const t = this
            t.dataBlocksFlow = []
            t.dataBlocksFakeFlow = []
            let data = {
                _mt: 'platAutoApprovalEditAgwService.getDataFlowBlock',
                aprdApprover: t.aprvrelaApproverid, // 员工端人id
                aprdBillid: t.aprdBillid, // 单据id
                aprvrelaStepid: t.aprvrelaStepidAll,
                aprvrelaRecordid: t.aprvrelaRecordidAll,
            ...
            getDataLevelUserLogin(data).then((res) => {
                if (isSuccess(res, t)) {
                    t.dataBlocksFakeFlow = res.data.content[0].dataBlocks
                    t.flowup = res.data.content[0].dataBlocks
                    t.blockStepId = res.data.content[0].stepId
                    t.flowName = res.data.content[0].stepName
                    t.flowId = res.data.content[0].flowId
  ...
  * */
            ...
            getDataLevelUserLogin(params).then((res) => {
                if (isSuccess(res, t)) {
                    ...
                    for (let i = 0; i < bb.length; i++) {
                        if (bb[i].id === dataBlockId && bb[i].id != "0") {
        ...
        getColumnFlow () {
            ...
            getDataLevelUserLogin(params).then((res) => {
                if (isSuccess(res, t)) {
                    t.dataBlocksFlow = [] // 进入之后每次清空datablock里的数据
                    t.finishCount = t.finishCount + 1
                    let bb = t.dataBlocksFakeFlow
                    for (let i = 0; i < bb.length; i++) {
                        bb[i]['blockColumn'] = res.data.content[0];
                        let tt = res.data.content[0].columns;
                        let obj = {
                            clmvIsmust: true,
                            clmvMod: "p_vldmod_000",
                            clmvName: "dimReceive",
                            clmvPrompts: "不能为空",
                            clmvValue: "",
                        }
                        tt[i].clmValue = ""
                        tt[i].columnValid.push(obj)
                        bb[i]['formlist'] = t.getFormDataSubmit(res.data.content[0].columns)
                    }
                    t.dataBlocksFakeFlow = bb // 临时block存储变量最后赋值给正式的block，这样才能正确更新数据
                    t.dataBlocksFlow = t.dataBlocksFakeFlow
                    for (let j = 0; j < res.data.content[0].columns.length; j++) {
                        if (res.data.content[0].columns[j].clmLayout == 13) {
                            t.clmkvMap[res.data.content[0].columns[j].clmName] = res.data.content[0].columns[j].clmDname
                        }
        ...
        getValueMap (dataBlocks) {
            const t = this
            t.valueMap = {}
            //var jsonStr=JSON.stringify(dataBlocks[0]);;
            for (let i = 0; i < dataBlocks.length; i++) {
                if (dataBlocks[i].blockColumn) {
