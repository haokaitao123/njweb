<template>
  <i-col class="col" span="16">
    <card :bordered="false">
      <p slot="title" class="title"><Icon type="android-notifications"></Icon> <span style="margin-left: 5px;">{{$t('lang_homePage.calendarList.title')}}</span></p>
      <a href="#" slot="extra" @click="openUp()">
        <Icon type="plus-round"></Icon>
      </a>
      <div class="block">
        <Row>
          <i-col span="12" style="font-size: 10px;">
            <v-calendar
              :attributes='attrs'
          ...
          <i-col span="11" offset="1">
            <div class="calendar_title">
              ...
              <div class="block_row"  v-for="(item , index) in sysmlvarComment" :key="index">

                <Icon class="icon" type="volume-high"  color="#f33155"></Icon>
                <div class="div"  @click="openUpdHome( item.id )">
                  {{item.remindTitle}}
                </div>
              </div>
          </i-col>
    ...
    <el-dialog title="创建日程" style="margin: 0 auto !important;"  width='850px' :visible.sync="openUpdate" @close="close">
      <addOrUpdate  :dateShow="dateShow" ref="addOrUpdate" :id="id" @closeTodo="close" @rerender="getData"></addOrUpdate>
    </el-dialog>
    <el-dialog title="修改日程" width='850px' :visible.sync="openUpdateHome" @close="close">
      <addOrUpdate ref="addOrUpdate" :id="id" :dateShow="dateShow" @closechang="close" @rerender="getData"></addOrUpdate>
    </el-dialog>
    <!-- <transition name="fade">
      <editCalendarList v-show="openUpdate"   :logType="logType"  @getData="getData"  @close="close" ref="editCalendarList"></editCalendarList>
    </transition> -->
    <transition name="fade">
      <calendarListPage v-if="openTable"  @close="close"  ref="calendarListPage"></calendarListPage>
    </transition>
    <!-- <transition name="fade">
