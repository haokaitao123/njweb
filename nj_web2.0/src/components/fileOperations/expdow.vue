    ...
    <div class="box">
      <div class="title">
        <div class="title-text">
          <Icon type="mouse" size="16" style="margin-right: 10px;"></Icon>&nbsp;{{$t('button.dwl')}}
        </div>
        <Button type="text" @click="closeExpDow">
          <Icon type="close-round" size="16"></Icon>
        </Button>
      </div>
      <Button type="text" v-if="status"  @click="dowFile">{{filename}}</Button>
      <strong style="color: red" v-if="!status">{{$t('reminder.noRightData')}}</strong>
      <br />
    </div>
  </div>
</template>
