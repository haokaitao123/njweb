    ...
    <div class="box" style="padding-bottom:20px;">
      <div class="title">
        <div class="title-text">
          <Icon type="mouse" size="16" style="margin-right: 10px;"></Icon>&nbsp;{{$t('button.exp')}}
        </div>
      ...
      <CheckboxGroup  v-model="expDisFields" @on-change="checkAllGroupChange" style="margin:10px 0 20px;">
        <Checkbox v-for="(item, index) in expDataTital" :key="index" :label="item.code+','+item.name" style="margin:10px;">{{item.name}}</Checkbox>
      </CheckboxGroup>
      <Button type="primary" @click="handleSubmit" style="float: right;">{{$t('button.exp')}}</Button>
    </div>
  </div>
</template>
<script>
