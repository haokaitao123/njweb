    ...
    <div class="box">
      <div class="title">
        <div class="title-text">
          <Icon type="mouse" size="16" style="margin-right: 10px;"></Icon>&nbsp;{{$t('button.imp')}}
        </div>
      ...
      <Row type="flex" justify="space-between">
        <i-col span="24" style="line-height: 32px;height: 40px;">
        	<span style="font-weight: bold;"></span>
          <Button type="ghost" icon="link" @click="dowFile">{{$t('lang_fileOperation.import.downTemplate')}}</Button>
        </i-col>
        <i-col span="24" style="line-height: 32px;height: 40px;" >
        ...
        <i-col span="24" style="margin-bottom: 10px;">
        	<div v-if="imptype == '1'" style="background: cornsilk;border-radius: 5px;padding: 12px;overflow: hidden;border: solid #FEEDD3 2px;opacity: 0.8;">
            {{$t('lang_fileOperation.import.addReminder')}}
        	</div>
        	<div v-if="imptype == '2'" style="background: cornsilk;border-radius: 5px;padding: 12px;overflow: hidden;border: solid #FEEDD3 2px;opacity: 0.8;">
            {{$t('lang_fileOperation.import.updReminder')}}
        	</div>
        </i-col>

        <i-col span="24" style="height: 60px;">
        	<Row>
        	...
        	<Row :gutter="40">
        		...
        		<i-col span="3">
		            ...
		            <Button type="ghost"  icon="ios-cloud-upload-outline">{{$t('lang_fileOperation.import.browsing')}}</Button>
              </Upload>
        		</i-col>
        ...
        <i-col span="24" style="height: 80px;margin-top:12px;">
        ...
        <i-col span="24" style="line-height: 32px;height: 40px;" v-if="imp_status !== 0">
          <Progress :stroke-width="5" :percent='imp_status' :status="activeState" :hide-info="true"></Progress>
        </i-col>
      </Row>
      </div>
    <transition name="fade">
      <importFail v-show="openImportFail" @closeImportFail="closeImportFail"ref="importFail"></importFail>
    </transition>
    </div>
  </div>
</template>
