    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="mouse" size="16" style="margin-right: 10px;"></Icon>&nbsp;{{$t('lang_organization.orgcostcenter.costcenterList')}}
        </div>
      ...
      <Page :total="total" :current="params.page" size="small" show-elevator show-sizer placement="top" @on-page-size-change="sizeChange" @on-change="pageChange" :page-size=params.rows :page-size-opts = "[10, 20, 50, 100]" ></Page>
    </div>
  </div>
</template>
<script>
  ...
  export default{
    ...
    methods: {
      ...
      dbCkick(row) {
        const t = this
        t.$emit('changeinput', row[t.searchCloumns[1].key], row.id)
        t.$emit('closeUp')
        this.close()
