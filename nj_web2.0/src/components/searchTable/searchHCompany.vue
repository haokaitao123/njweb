    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="mouse" size="16" style="margin-right: 10px;"></Icon>&nbsp;{{$t('lang_employee.searchColumn.searchCompany')}}
        </div>
      ...
      <row class="table-form" ref="table-form">
        <Table height="320" size="small" border ref="selection" :columns="searchHCClo" @on-sort-change="sortable" :data="data" @on-row-dblclick="dbCkick"></Table>
      </row>
    ...
    methods: {
      search() {
    	...
    	dbCkick(row) {
    		const t = this
        this.close()
	        t.$emit('inputHCompany', row.unitsName, row.id)
	        t.$emit('closeHCompany')
    	},
    	sortable(column) {
