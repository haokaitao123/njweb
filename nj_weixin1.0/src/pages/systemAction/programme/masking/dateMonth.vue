...
<script>
  export default {
    ...
    mounted(){
      let year = document.getElementsByClassName('mu-date-display-year-title')[1].innerHTML
      let monthday = document.getElementsByClassName('mu-date-display-monthday-title')[1].innerHTML
      this.year = year
    ...
    methods:{
      ...
      dateSure() {
        if(this.$store.state.start == '开始时间'){
          ...
          this.$store.commit('dateMonthreveal',false)
          this.$store.commit('selectDateTimeStart',this.SelectdateTime)
          ...
          console.log(this.SelectdateTime)
          this.$store.commit('dateMonthreveal',false)
          this.$store.commit('selectDateTimeEnd',this.SelectdateTime)
        }
