<template>
  <div class="wrap" style="overflow: hidden">
    ...
    <memberResume :dataList="dataList" style="height: auto;"></memberResume>
    <mu-container v-show="showContainer" center style="">
      <mu-tabs :value.sync="active" color="#FD9DBD" :inverse='true' style="display: flex;overflow-x: scroll"  class="tabs">
        ...
        <mu-tab style="font-size: 15px">地址信息</mu-tab>
        <mu-tab style="font-size: 15px">家庭成员</mu-tab>
        <mu-tab style="font-size: 15px">工作履历</mu-tab>
      </mu-tabs>
    </mu-container>
    <div class="teamDetail">
      ...
      <div class="demo-text" v-if="active === 5">
        <memberAddress :empId="id"></memberAddress>
      </div>
      <div class="demo-text" v-if="active === 6">
        <memberfamily :empId="id"></memberfamily>
      </div>
      <div class="demo-text" v-if="active === 7">
        <memberWorkexp :empId="id"></memberWorkexp>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  import memberfamily from './memberfamily.vue'
  import memberWorkexp from './memberWorkexp.vue'
  ...
  export default {
    ...
    mounted() {
      this.id = localStorage.getItem('bus_empid')
      this.getData()
    },
    methods:{
      getData(){
